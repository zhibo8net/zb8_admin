<template>
  <div>
      <h2>新闻管理</h2>
      <div><button type="button" class="btn btn-info pull-right" v-on:click="create()">新建新闻</button></div>
      <table class="table">
        <thead>
          <tr>
            <th>标题</th>
...
export default {
  ...
  methods: {
    ...
    goto: function (page) {
      var that = this
      Vue.http.get(g.t('/api/admin/newss/page?page=' + page)).then(function (response) {
